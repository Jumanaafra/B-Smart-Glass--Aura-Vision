...
import { userAPI, BACKEND_URL, getHeaders, authAPI, clearLocalAuth } from '../../utils/api';
...
export const SettingsScreen: React.FC<SettingsScreenProps> = ({ setPage }) => {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      try {
        // Use userAPI helper (reads from VITE_BACKEND_URL)
  ...
  const handleLogout = async () => {
    // Must call backend to clear httpOnly cookie (JS cannot do this itself)
    try { await authAPI.logout(); } catch (_) { /* proceed anyway */ }
    clearLocalAuth();
